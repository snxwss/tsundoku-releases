...
## 1.7.2-beta — 2026-09-19

- Switching the interface language no longer changes how visual novel titles are shown. Titles follow only the *Title language* setting, as before 1.7.1.
...
## 1.6.5-beta — 2026-09-13

- Removed the thin empty strip that 1.6.4 left along the right edge of Browse, Wishlist and Library. Spacing is back to how it looked before, and the resizing fix still holds.
...
- Fixed character art and screenshots staying blank for a title even after viewing it: the screenshot list was never saved, and a failed load could leave behind an empty entry that then looked like a real "nothing here" answer forever.
- Cached content now appears instantly instead of only being used after a failed request.
- Connection errors now say what actually went wrong (e.g. no internet) instead of always reporting a timeout.
...
- Fixed the hard content-block being too aggressive: it now only excludes a title when a blocked category is strongly present, instead of triggering on a weak/incidental tag application — this was wrongly hiding some acclaimed, mainstream titles.
- Adjusted the extreme-content tag list based on review.
- Tag chips on a title's detail page are now clickable and open that tag's VNDB page.
- Blurred covers in Scan Results can now be clicked to reveal them.
